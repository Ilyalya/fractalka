...
def fractal_dimension(immat, ws=range(1, 10)) -> float:
    """
    Вычислить классическую фрактальную размерность двоичного изображения
    :param immat: двоичное изображение
    :param ws:    размер подмножеств изображения
    :rparam:      вернуть фрактальную размерность
    """
...
def reni_entropy(p: float, q: int):
    """
    Вычислить энтропию рени
    :param p: вектор вероятностей
    :param q: целое число, которое позволяет этой энтропии быть универсальной
    """
    if q != 1:
